...
ms-swift `4.4.1`、`4.5.0.dev0` 和 `4.6.0.dev0`。新的训练环境推荐直接使用
[ms-swift 官方仓库](https://github.com/modelscope/ms-swift)；PR
[#10012](https://github.com/modelscope/ms-swift/pull/10012) 已将 AgentArk agentic 多步
rollout 所需的精确 token-in/token-out 修复合入官方 `main`。
...
不要省略 `-e "$SWIFT_ROOT"`，否则 Swift 可能加载环境中其他版本的源码。

这套版本用于 Linux、NVIDIA CUDA 和 vLLM colocate。若硬件需要其他 Torch/vLLM wheel，
仍应使用上面列出的某个已验证 ms-swift 版本，按照相应 CUDA 兼容关系安装，并重新执行
本页 smoke；launcher 会拒绝未经验证的版本。
...
| `AGENTARK_USE_VLLM` | `true` | 是否使用 vLLM rollout；`false` 使用 Transformers |
...
- 当前 adapter 和 rollout cleanup 验证于 ms-swift `4.4.1`、`4.5.0.dev0` 与
  `4.6.0.dev0`；新的 agentic 多步训练推荐使用官方仓库。
- 已验证运行平台为 Linux、NVIDIA CUDA、单机 vLLM colocate。
- bundled launcher 提供单机多卡参数和预检；仓库 smoke 回归基线为单卡，Snake 教程
  另外记录了一次完整的 8 卡实跑。多节点和 vLLM server mode 需要额外的容量与路由实现。
...
- Swift Gym 环境接口：
  ...
  （用于理解 Gym API；本接入的具体兼容行为以 launcher 的版本检查为准）
