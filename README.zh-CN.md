...
<p align="center">
  <img src="docs/figures/agentark-figure1.png" alt="AgentArk 概览" width="900">
</p>
...
- 使用 verl 进行强化学习训练：[docs/rl-training.zh-CN.md](docs/rl-training.zh-CN.md)
...
AgentArk 提供环境服务端。verl GRPO 集成位于公开 fork
[P90-RushB/verl](https://github.com/P90-RushB/verl) 的 `agentark_rl` 分支：
[`agentark_recipe/agentark_env_agent`](https://github.com/P90-RushB/verl/tree/agentark_rl/agentark_recipe/agentark_env_agent)。

运行时 wrapper 和环境服务在 AgentArk 的 Python 3.10.12 环境中运行。verl trainer
通过 HTTP 与 AgentArk 通信，因此可以使用自己的 Python 环境。

在 AgentArk checkout 中启动环境服务：
...
然后按照 verl 集成指南生成数据集并启动 GRPO 训练：
...
- `agent_ark.ark_rl`：为未来包内 RL adapter 预留的命名空间；当前 RL 训练实现在
  verl fork 中。
